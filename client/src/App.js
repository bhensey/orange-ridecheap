...
class App extends Component {
  constructor(props) {
    ...
    this.state = {
      results: [],
      loading: false,
      center: { lat: 42.057858, lng: -87.675837 },
      zoom: 11,
      Error: false,
      selectedStart: null,
      selectedEnd: null
  ...
  onGo(start, end) {
    ...
    let promiseArray = [];
    promiseArray.push(
      callRideshare(process.env.REACT_APP_SERVER + "/api/v1/uber", start, end)
    );
    promiseArray.push(
      callRideshare(process.env.REACT_APP_SERVER + "/api/v1/lyft", start, end)
    );
    ...
    return (
      <MuiThemeProvider theme={theme}>
        <div className="App">
          ...
          />
          {this.state.loading ? (
            <CircularProgress className="loader" size={80} color="secondary" />
          ) : (
